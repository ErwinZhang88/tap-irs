...
public function assets_price(Request $request) {
		$proxyhost = '';
		$proxyport = '';
		$proxyusername = '';
		$proxypassword = '';
		$client = new nusoap_client( 
			'http://10.20.1.140:8000/sap/bc/soap/wsdl11?services=ZFMAMS_ASSETS_PRICE&sap-client=700', 
			'wsdl',
			$proxyhost, 
			$proxyport, 
			$proxyusername, 
		...
		$result = $client->call('ZFMAMS_ASSETS_PRICE', array('parameters' => $param), '', '', false, true);
		if ($client->fault) {
			echo '<h2>Fault</h2><pre>';
			print_r($result);
			echo '</pre>';
