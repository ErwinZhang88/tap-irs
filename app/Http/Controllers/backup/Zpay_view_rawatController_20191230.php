...
{	
    public function index( $comp_ba )
	{
		...
		$sql = "SELECT prfnr profile_name,
					   ...
					   reasn reason,
					   ernam created_by,
					   TO_DATE (erdat, 'YYYYMMDD') created_on,
		...
		{	
			...
			foreach( $datax as $k)
			{
				...
				$data3 = array(
                    ...
                    'phase' => $k->phase,  
                    'item' => $k->item,
                    'total_item' => $k->total_item,
                    'activity' => $k->activity,  
                    'activity_name' => $k->activity_name,  
                    'block' => $k->block,   
                    'old_block' => $k->old_block,
                    'mandor_code' => $k->mandor_code,
                    'helper1_code' => $k->helper1_code,
                    'helper1_name' => $k->helper1_name,
                    'helper2_code' => $k->helper2_code,
                    'helper2_name' => $k->helper2_name,
                    'helper3_code' => $k->helper3_code,
                    'helper3_name' => $k->helper3_name,
                    'per' => $k->per,
                    'customer' => $k->customer,
                    'activity_uom' => $k->activity_uom,                    
                    'vehicle_license_no' => $k->vehicle_license_no,
                    'start_mileage' => $k->start_mileage,
                    'end_mileage' => $k->end_mileage,
                    'mileage' => $k->mileage,
                    'uom' => $k->uom,
                    'asset' => $k->asset,
                    'asset_maint_no' => $k->asset_maint_no,
                    'cost_center' => $k->cost_center,
                    'doc_currency' => $k->doc_currency,
                    'premi_rate' => $k->premi_rate,
                    'over_time' => $k->over_time,
                    'mat_1' => $k->mat_1,
                    'mat_desc1' => $k->mat_desc1,
                    'mat_qty1' => $k->mat_qty1,
                    'mat_uom1' => $k->mat_uom1,
                    'mat_2' => $k->mat_2,
                    'mat_desc2' => $k->mat_desc2,
                    'mat_qty2' => $k->mat_qty2,
                    'mat_uom2' => $k->mat_uom2,
                    'mat_3' => $k->mat_3,
                    'mat_desc3' => $k->mat_desc3,
                    'mat_qty3' => $k->mat_qty3,
                    'mat_uom3' => $k->mat_uom3,
                    'plant' => $k->plant,
                    'reason' => $k->reason,
                    ...
                    'time_of_changed' => $k->time_of_changed
				);
